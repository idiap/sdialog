    ...
    if isinstance(input_dialogues, str) and os.path.isdir(input_dialogues):
        ...
        for filename in tqdm(os.listdir(path_dialogues), desc="Dialogues:"):
            ...
            if ext in [".json", ".txt"]:  # "csv", "tsv"
                input_dialogues.append(Dialog.from_file(os.path.join(path_dialogues, filename)))
                if input_dialogues[-1].id is None:
                    input_dialogues[-1].id = dialog_id
            else:
